    ...
    for det in out_car:
        for box in det.boxes:
            ...
            if vehicle_img.shape[0] > 0 and vehicle_img.shape[1] > 0:
                ...
                # color detection for all classes
...
if not cap.isOpened():
    print("ERROR >> Webcam not found/opened!")
    exit()
...
while True:
    ...
    fps_text = "FPS: {:.2f}".format(1/elapsed_time if elapsed_time != 0 else 0)
...
video_writer = cv2.VideoWriter(os.path.join(PATHS["output"], output_videoname),
                               fourcc, fps, (frame_width, frame_height))

# Add this line after cv2.imshow:
